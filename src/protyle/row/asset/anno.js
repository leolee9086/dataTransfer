...
const rectAnno = (config, pdf, element) => {
    ...
    config.mainContainer.addEventListener("mousedown", (event) => {
        ...
        documentSelf.onmousemove = (moveEvent) => {
            ...
            rectResizeElement.setAttribute("style", `top:${newTop}px;height:${newHeight}px;left:${newLeft}px;width:${newWidth}px;`);
        };
    ...
    element.addEventListener("click", (event) => {
        ...
        while (target && !target.classList.contains("pdf__outer")) {
            ...
            if (target.classList.contains("b3-color__square")) {
                ...
                writeText(`<<${pdf.appConfig.file.replace(location.origin, "").substr(1)}/${rectElement.getAttribute("data-node-id")} "${rectElement.getAttribute("data-content")}">>`);
                hideToolbar(element);
                event.preventDefault();
                event.stopPropagation();
                processed = true;
...
const getHightlightCoordsByRect = (pdf, color, rectResizeElement) => {
...
const getHightlightCoordsByRange = (pdf, color) => {
    ...
    if (endSelected.length > 0) {
        ...
        results.push({ index: endIndex, coords: endSelected, id, color, content, type: "text" });
    }
    if (pages.length === 0) {
        return;
    }
