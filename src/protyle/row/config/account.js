...
<div class="config-account__center config-account__center--text${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}">
    ...
    bindEvent: () => {
        ...
        const agreeLoginElement = account.element.querySelector("#agreeLogin");
        const userNameElement = account.element.querySelector("#userName");
        if (!userNameElement) {
            ...
            account.element.querySelectorAll("input[type='checkbox']").forEach(item => {
                item.addEventListener("change", () => {
                    fetchPost("/api/setting/setAccount", {
                        displayTitle: account.element.querySelector("#displayTitle").checked,
                        displayVIP: account.element.querySelector("#displayVIP").checked,
                    }, (response) => {
            ...
            activationCodeElement.addEventListener("click", () => {
                fetchPost("/api/account/checkActivationcode", { data: activationCodeElement.previousElementSibling.value }, (response) => {
        ...
        const userPasswordElement = account.element.querySelector("#userPassword");
        const captchaImgElement = account.element.querySelector("#captchaImg");
        const captchaElement = account.element.querySelector("#captcha");
        const twofactorAuthCodeElement = account.element.querySelector("#twofactorAuthCode");
        const loginBtnElement = account.element.querySelector("#login");
        const login2BtnElement = account.element.querySelector("#login2");
