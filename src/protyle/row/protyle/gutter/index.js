...
export class Gutter {
    constructor(protyle) {
        ...
        this.element.addEventListener("click", (event) => {
            ...
            if (!id) {
                const gutterFold = () => {
                    ...
                    const foldElement = protyle.wysiwyg.element.querySelector(`[data-node-id="${(buttonElement.previousElementSibling || buttonElement.nextElementSibling).getAttribute("data-node-id")}"]`);
                ...
                if (protyle.disabled) {
                    confirmDialog(window.siyuan.languages["_kernel"]["34"], window.siyuan.languages.foldTip, () => {
                        if (isMobile()) {
                            document.getElementById("toolbarName").readOnly = false;
            ...
            if (window.siyuan.ctrlIsPressed) {
                ...
                const foldElement = protyle.wysiwyg.element.querySelector(`[data-node-id="${id}"]`);
        ...
        this.element.addEventListener("mouseover", (event) => {
            const buttonElement = hasClosestByTag(event.target, "BUTTON");
            if (!buttonElement || (buttonElement === null || buttonElement === void 0 ? void 0 : buttonElement.getAttribute("data-type")) === "fold") {
                return;
            }
            let nodeElement;
    ...
    renderMenu(protyle, buttonElement) {
        ...
        window.siyuan.menus.menu.append(new MenuItem({
            ...
            icon: "iconCopy",
            type: "submenu",
            submenu: copySubMenu(id, (_a = nodeElement.querySelector(".protyle-attr--name")) === null || _a === void 0 ? void 0 : _a.textContent, true, nodeElement).concat([{
                    label: window.siyuan.languages.copy,
                    accelerator: "⌘C",
                    click() {
                    ...
                    click() {
                        const tempElement = nodeElement.cloneNode(true);
...
<input style="margin: 4px 0;width: 84px" type="number" step="1" min="148" class="b3-text-field" value="${height ? parseInt(height) : "420"}">`,
                        bind: (element) => {
                            element.querySelector("input").addEventListener("change", (event) => {
                                ...
                                nodeElement.firstElementChild.style.height = newHeight;
        ...
        if (!["NodeThematicBreak", "NodeBlockQueryEmbed", "NodeIFrame", "NodeHTMLBlock", "NodeWidget", "NodeVideo", "NodeAudio"].includes(type) &&
            ((_b = getContenteditableElement(nodeElement)) === null || _b === void 0 ? void 0 : _b.textContent.trim()) !== "" &&
