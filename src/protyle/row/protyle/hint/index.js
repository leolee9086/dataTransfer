...
import { focusByRange, focusByWbr, focusSideBlock, getEditorRange, getSelectionOffset, getSelectionPosition } from "../util/selection";
...
${unicode2Emoji(emoji.unicode, true)}</button>`;
            ...
            if (emojiElement) {
                ...
                if (this.element.querySelectorAll(".emojis__title").length > 2) {
                    ...
                    if (range.endContainer.nodeType !== 3) {
                        (_a = range.endContainer.childNodes[range.endOffset - 1]) === null || _a === void 0 ? void 0 : _a.remove();
                    }
                    addEmoji(unicode);
                    let emoji;
    ...
    render(protyle) {
        ...
        const range = getSelection().getRangeAt(0);
        const start = getSelectionOffset(range.startContainer, protyle.wysiwyg.element).start;
...
<div class="fn__flex${value ? " fn__none" : ""}">
    ...
    select(event, protyle) {
        ...
        if (isEmojiPanel) {
            ...
            if (event.key === "ArrowLeft" || event.key === "ArrowUp") {
                ...
                else if ((_b = currentElement.parentElement.previousElementSibling) === null || _b === void 0 ? void 0 : _b.previousElementSibling) {
                    currentElement.classList.remove("emojis__item--current");
                ...
                else if ((_c = currentElement.parentElement.nextElementSibling) === null || _c === void 0 ? void 0 : _c.nextElementSibling) {
                    currentElement.classList.remove("emojis__item--current");
    ...
    getKey(currentLineValue, extend) {
        ...
        // 冒号前为数字或冒号不进行emoji提示
        if (this.splitChar === ":" && (/\d/.test(currentLineValue.substr(this.lastIndex - 1, 1)) ||
