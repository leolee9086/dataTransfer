...
export class Search extends Model {
    ...
    <div class="fn__flex-1" id="searchPreview"></div>
        ...
        const historyElement = this.element.querySelector("#searchHistoryList");
    ...
    getArticle(id, value) {
        fetchPost("/api/block/checkBlockFold", { id }, (foldResponse) => {
            if (this.protyle) {
                ...
                this.protyle = new Protyle(this.element.querySelector("#searchPreview"), {
                    blockId: id,
                    hasContext: !foldResponse.data,
                    key: value,
                    render: {
    ...
    setLocalStorage(value) {
        ...
        let searches = JSON.parse(localStorage.getItem(Constants.LOCAL_SEARCHETABDATA) || "[]");
    ...
    <span class="b3-list-item__meta b3-list-item__meta--ellipsis" title="${Lute.EscapeHTMLStr(title)}">${Lute.EscapeHTMLStr(title)}</span>
        ...
        this.getArticle(data[0].id, this.element.querySelector(".b3-text-field").value);
    }
}
