...
let protyle;
export const openSearch = (hotkey, key, notebookId, searchPath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const searchFilterElement = dialog.element.querySelector("#searchFilter");
    const searchFilterPanelElement = dialog.element.querySelector("#searchFilterPanel");
    ...
    searchFilterPanelElement.querySelectorAll("input").forEach(item => {
        item.addEventListener("change", () => {
            inputEvent();
            localData.types = {
                document: dialog.element.querySelector("#document").checked,
                heading: dialog.element.querySelector("#heading").checked,
                list: dialog.element.querySelector("#list").checked,
                listItem: dialog.element.querySelector("#listItem").checked,
                codeBlock: dialog.element.querySelector("#codeBlock").checked,
                htmlBlock: dialog.element.querySelector("#htmlBlock").checked,
                mathBlock: dialog.element.querySelector("#mathBlock").checked,
                table: dialog.element.querySelector("#table").checked,
                blockquote: dialog.element.querySelector("#blockquote").checked,
                superBlock: dialog.element.querySelector("#superBlock").checked,
                paragraph: dialog.element.querySelector("#paragraph").checked,
            };
            localStorage.setItem(Constants.LOCAL_SEARCHEDATA, JSON.stringify(localData));
        });
    });
    ...
    const searchInputElement = dialog.element.querySelector("#searchInput");
    const replaceInputElement = dialog.element.querySelector("#replaceInput");
    ...
    replaceHistoryElement.addEventListener("click", (event) => {
        if (event.target.classList.contains("b3-list-item")) {
    ...
    dialog.element.querySelector("#replaceHistoryBtn").addEventListener("click", () => {
        let html = "";
        (localData.replaceList || []).forEach((s) => {
    ...
    const replace = (isAll) => {
        ...
        let searches = localData.replaceList || [];
    ...
    let clickTimeout;
    searchPanelElement.addEventListener("click", (event) => {
...
const getArticle = (options) => {
    fetchPost("/api/block/checkBlockFold", { id: options.id }, (foldResponse) => {
        if (!protyle) {
            protyle = new Protyle(options.dialog.element.querySelector("#searchPreview"), {
                blockId: options.id,
                hasContext: !foldResponse.data,
                key: options.k,
                render: {
...
<span class="b3-list-item__meta b3-list-item__meta--ellipsis" title="${Lute.EscapeHTMLStr(title)}">${Lute.EscapeHTMLStr(title)}</span>
    ...
    if (data[0]) {
        ...
        getArticle({
            dialog,
            id: data[0].id,
            k: dialog.element.querySelector("input").value,
        });
    }
    else {
        if (protyle) {
