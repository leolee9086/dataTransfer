    ...
    window.addEventListener("keydown", (event) => {
        ...
        if (event.ctrlKey && !event.metaKey && event.key === "Tab") {
            ...
            if (allTabs[currentIndex].model instanceof Editor) {
                ...
                allTabs[currentIndex].model.editor.protyle.wysiwyg.element.dispatchEvent(new CustomEvent("focusout"));
            }
            if (event.shiftKey) {
                if (currentIndex === 0) {
        ...
        if (matchHotKey(window.siyuan.config.keymap.general.closeTab.custom, event)) {
            ...
            if (activeTabElement && activeTabElement.getBoundingClientRect().width > 0) {
                const type = activeTabElement.parentElement.className.split("sy__")[1];
            ...
            getAllTabs().find(item => {
                var _a;
                if ((_a = item.headElement) === null || _a === void 0 ? void 0 : _a.classList.contains("item--focus")) {
...
const fileTreeKeydown = (event) => {
    ...
    if (matchHotKey("⌘/", event)) {
        ...
        if (isFile) {
            ...
            initNavigationMenu(liElement).popup({ x: liRect.right - 15, y: liRect.top + 15 });
        }
        return true;
    }
    ...
    if (event.key === "Delete" || (event.key === "Backspace" && isMac())) {
        if (isFile) {
            ...
            confirmDialog(window.siyuan.languages.delete, `${window.siyuan.languages.confirmDelete} <b>${Lute.EscapeHTMLStr(getNotebookName(notebookId))}</b>?`, () => {
