...
export const highlightById = (protyle, id, top = false) => {
    ...
    Array.from(protyleElement.querySelectorAll(`[data-node-id="${id}"]`)).find((item) => {
...
export const scrollCenter = (protyle, nodeElement, top = false) => {
    ...
    if (!nodeElement) {
        nodeElement = hasClosestBlock(getEditorRange(protyle.wysiwyg.element).startContainer);
    }
    if (!nodeElement) {
        return;
    }
    ...
    while (!parentNodeElement.classList.contains("protyle-wysiwyg")) {
        offsetTop += parentNodeElement.offsetTop;
