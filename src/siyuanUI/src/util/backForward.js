...
const focusStack = (stack) => __awaiter(void 0, void 0, void 0, function* () {
    hideElements(["gutter", "toolbar", "hint", "util", "dialog"], stack.protyle);
    let blockElement;
    if (!stack.protyle.element.parentElement) {
        const response = yield fetchSyncPost("/api/block/checkBlockExist", { id: stack.protyle.block.rootID });
        if (!response.data) {
            // 页签删除
            return false;
        }
        ...
        if (wnd) {
            const info = yield fetchSyncPost("/api/block/getBlockInfo", { id: stack.id });
            if (info.code === 2) {
                // 文件被锁定
                lockFile(info.data);
                return false;
    ...
    if (stack.protyle.element.parentElement) {
        const response = yield fetchSyncPost("/api/block/checkBlockExist", { id: stack.id });
        if (!response.data) {
            // 块被删除
...
export const goBack = () => __awaiter(void 0, void 0, void 0, function* () {
    if (window.siyuan.backStack.length === 0) {
        if (forwardStack.length > 0) {
            yield focusStack(forwardStack[forwardStack.length - 1]);
        }
        return;
    }
    ...
    while (stack) {
        const isFocus = yield focusStack(stack);
        if (isFocus) {
...
export const goForward = () => __awaiter(void 0, void 0, void 0, function* () {
    if (forwardStack.length === 0) {
        if (window.siyuan.backStack.length > 0) {
            yield focusStack(window.siyuan.backStack[window.siyuan.backStack.length - 1]);
        }
        return;
    }
